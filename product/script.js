...
let addressMapLockUntil = 0;
const ADDRESS_MAP_LOCK_MS = 15 * 60 * 1000;
...
if (addressField) {
    addressField.addEventListener("click", () => {
        ...
        if (now >= addressMapLockUntil) {
            addressMapLockUntil = now + ADDRESS_MAP_LOCK_MS;
            openMapPopup();
        }
    });
}
...
function bindMapUiControls() {
    ...
    document.getElementById("enterManuallyBtn")?.addEventListener("click", () => {
        ...
        if (addressField) {
            addressField.focus();
            addressField.setSelectionRange(
                addressField.value.length,
                addressField.value.length
            );
        }
    });
