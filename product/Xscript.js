...
function populateVehicleTypeOptions(vehicleTypes = [], selectedValue = "") {
    ...
    const optionsMarkup = [`<option value="">All type vehicle</option>`]
    ...
    if (dropdown) {
        ...
        dropdown.disabled = !hasVehicleTypes;
        dropdown.style.display = hasVehicleTypes ? "block" : "none";
        dropdown.value = vehicleTypes.includes(normalizedSelected) ? normalizedSelected : "";
        dropdown.classList.toggle("vehicle-type-prompt", hasVehicleTypes && !dropdown.value);
    }

    if (popupDropdown) {
        popupDropdown.innerHTML = optionsMarkup;
        popupDropdown.disabled = !hasVehicleTypes;
        popupDropdown.style.display = hasVehicleTypes ? "block" : "none";
        popupDropdown.value = vehicleTypes.includes(normalizedSelected) ? normalizedSelected : "";
        popupDropdown.classList.toggle("vehicle-type-prompt", hasVehicleTypes && !popupDropdown.value);
    }
}
    ...
    return new Promise((resolve) => {
        ...
        confirmBtn.onclick = () => {
            ...
            if (selectedCity === "Other") {
                ...
                saveVehicleType("");
                cleanup();
                resolve({ type: "other", city: typedCity, vehicleType: "" });
                return;
            }

            saveConfirmedCity(selectedCity);
            saveVehicleType(vehicleTypeSelect?.value || "");
            cleanup();
            resolve({ type: "city", city: selectedCity, vehicleType: vehicleTypeSelect?.value || "" });
        };
